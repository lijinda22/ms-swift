...
class CompatArguments:
    ...
    def _handle_ckpt_dir(self: "BaseArguments"):
        ...
        if (
            os.path.exists(os.path.join(self.ckpt_dir, "adapter_config.json"))
            or os.path.exists(
                os.path.join(self.ckpt_dir, "default", "adapter_config.json")
            )
...
class BaseArguments(
    ...
    train_type: str = field(
        default="lora",
        metadata={"help": f"train_type choices: {list(get_supported_tuners())}"},
    )
    ...
    use_swift_lora: bool = (
        False  # True for using tuner_backend == swift, don't specify this unless you know what you are doing # noqa
    )
    ...
    def _check_is_adapter(adapter_dir: str) -> bool:
        if (
            ...
            or os.path.exists(
                os.path.join(adapter_dir, "default", "adapter_config.json")
            )
    ...
    def _init_adapters(self):
        ...
        self.adapters = [
            safe_snapshot_download(
                adapter, use_hf=self.use_hf, hub_token=self.hub_token
            )
    ...
    def __post_init__(self):
        ...
        self.rank, self.local_rank, self.global_world_size, self.local_world_size = (
            get_dist_setting()
        )
        logger.info(
    ...
    def _init_ckpt_dir(self, adapters=None):
        ...
        model = (
            self.model
            or getattr(self, "mcore_model", None)
            or getattr(self, "load", None)
        )
    ...
    def save_args(self, output_dir=None) -> None:
        if is_master():
            ...
            with open(fpath, "w", encoding="utf-8") as f:
                json.dump(
                    check_json_format(self.__dict__), f, ensure_ascii=False, indent=2
                )
    ...
    def get_template(
        self, processor: Optional["Processor"], template_type: Optional[str] = None
    ) -> "Template":
