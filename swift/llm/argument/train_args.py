    ...
    def _init_eval_strategy(self):
        ...
        if self.eval_strategy == "no":
            ...
            if self.split_dataset_ratio > 0:
                ...
                logger.info(
                    f"Setting args.split_dataset_ratio: {self.split_dataset_ratio}"
                )
    ...
    def _init_metric(self):
        ...
        if self.metric_for_best_model is None:
            self.metric_for_best_model = (
                "rouge-l" if self.predict_with_generate else "loss"
            )
...
class SwanlabArguments:
    ...
    def _init_swanlab(self):
        if not is_swanlab_available():
            raise ValueError(
                "You are using swanlab as `report_to`, please install swanlab by "
                "`pip install swanlab`"
            )
...
class TrainArguments(
    SwanlabArguments, TunerArguments, BaseArguments, Seq2SeqTrainingOverrideArguments
):
    """
    ...
    kd_teacher_model_type: Optional[List[str]] = field(
        default=None,
        metadata={"help": 'Type of the teacher model for ViT KD (e.g., "conchv1_5")'},
    )
    kd_teacher_model_path: Optional[List[str]] = field(
        default=None,
    ...
    def _check_padding_free(self):
        if self.padding_free or self.packing:
            ...
            if self.attn_impl not in {
                ...
                raise ValueError(
                    f'The "{feature}" feature requires a flash attention implementation. '
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.dataset) == 0 and len(self.cached_dataset) == 0:
            raise ValueError(
                f"self.dataset: {self.dataset}, self.cached_dataset: {self.cached_dataset}. "
    ...
    def _init_deepspeed(self):
        if self.deepspeed:
            ...
            if is_mp():
                raise ValueError(
                    "DeepSpeed is not compatible with `device_map`. "
            ...
            if self.zero_hpz_partition_size is not None:
                ...
                ] = self.zero_hpz_partition_size
                logger.warn(
                    "If `zero_hpz_partition_size`(ZeRO++) causes grad_norm NaN, please"
            ...
            if self.deepspeed_autotp_size is not None:
                ...
                self.deepspeed["tensor_parallel"] = {
                    "autotp_size": self.deepspeed_autotp_size
                }
                self.deepspeed["zero_optimization"][
