...
class GRPOConfig(GRPOArgumentsMixin, SwiftArgumentsMixin, HfGRPOConfig):
    ...
    def __post_init__(self):
        ...
        if self.vllm_reasoning_parser is not None:
            raise ValueError(
                "vllm_reasoning_parser is not supported for GRPO Training, please unset it."
            )
        ...
        if (
            self.deepspeed
            and "zero_optimization" in self.deepspeed
            and self.deepspeed["zero_optimization"]["stage"] == 3
        ):
            # https://github.com/modelscope/ms-swift/issues/3237
            self.deepspeed["zero_optimization"]["stage3_prefetch_bucket_size"] = 0
            self.deepspeed_plugin.hf_ds_config.config["zero_optimization"][
