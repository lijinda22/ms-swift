...
class SwiftRLHF(SwiftSft):
    ...
    def _get_model_task_type(model_dir):
        ...
        if os.path.exists(os.path.join(model_dir, "args.json")):
            ...
            if hasattr(model_config, "architectures") and model_config.architectures:
                if any(
                    "sequenceclassification" in arch.lower()
    ...
    def _prepare_model_tokenizer(self):
        ...
        for key in ["ref", "value", "teacher"]:
            ...
            result = self._prepare_single_model(
                model_key, key, model_type, model_revision
            )
        ...
        if hasattr(args, "reward_model") and args.reward_model is not None:
            ...
            for reward_model_path, rm_type, rm_revision in zip(
                ...
                result = self._prepare_single_model(
                    "reward", None, rm_type, rm_revision
                )
                if result is not None:
    ...
    def prepare_model(
        ...
        if args.ref_adapters:
            if args.train_type in extra_tuners:
                ...
                tuner = Swift
            assert (
                len(args.ref_adapters) == 1
            ), f"args.ref_adapters: {args.ref_adapters}"
            model = tuner.from_pretrained(
                model, args.ref_adapters[0], adapter_name="ref_adapter"
            )
            assert args.rlhf_type in {
                "dpo",
                "kto",
                "grpo",
            }, "Currently, only DPO, KTO, and GRPO support `ref_adapters`."
    ...
    def _prepare_chord_sft_dataset(self):
        ...
        chord_sft_dataset, _ = load_dataset(
            args.chord_sft_dataset,
            split_dataset_ratio=0,
            shuffle=args.dataset_shuffle,
            **dataset_kwargs,
        )
        chord_sft_dataset, _ = self._encode_dataset(
            chord_sft_dataset, None, pre_process=True
        )
    ...
    def _get_trainer_kwargs(self):
        ...
        if self.args.rlhf_type == "grpo":
            ...
            if self.args.chord_sft_dataset:
                trainer_kwargs["chord_sft_dataset"], _ = (
                    self._prepare_chord_sft_dataset()
                )
