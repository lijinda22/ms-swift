...
class ShareGPT4oPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset):
        if not use_hf_hub():
            url = (
                "https://www.modelscope.cn/api/v1/datasets/AI-ModelScope/ShareGPT-4o/repo?"
        ...
        self.prefix_path = os.path.join(
            local_dir, "mnt", "petrelfs", "wangwenhai", "workspace_cef", "4o", "image"
        )
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=GPT4vDataset(columns={"link": "images", "caption": "response"}),
        split=["train"],
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=GarbagePreprocessor(
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=SA1BPairedCaptionPreprocessor(
            columns={
...
class MantisPreprocessor(MessagesPreprocessor):
    ...
    def __init__(
        self, *, subset: str, columns: Optional[Dict[str, str]] = None
    ) -> None:
        self.subset = subset
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        if not use_hf_hub():
            url = (
                f"https://www.modelscope.cn/api/v1/datasets/swift/Mantis-Instruct/repo?Revision="
                f"master&FilePath={self.subset}/train_images.zip"
            )  # noqa
        else:
            url = (
                f"{get_hf_endpoint()}/datasets/TIGER-Lab/Mantis-Instruct/"
...
for subset in mantis_subsets_name:
    _subset = SubsetDataset(
        subset=subset,
        split=["train"],
        preprocess_func=MantisPreprocessor(subset=subset),
    )
...
class LLaVADataPreprocessor(MessagesPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for image in images:
            if "coco/" in image:
                image = os.path.join(
                    self.all_folders["coco"], image.replace("coco/", "")
                )
            elif "gqa/" in image:
                ...
                image = os.path.join(
                    self.all_folders["textvqa"], image.replace("textvqa/", "")
                )
            elif "VG_100K/" in image:
                image = os.path.join(
                    self.all_folders["VG_100K"], image.replace("vg/", "")
                )
            elif "VG_100K_2/" in image:
                image = os.path.join(
                    self.all_folders["VG_100K_2"], image.replace("vg/", "")
                )
...
class EmoSchemaPreprocessor(ResponsePreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        ...
        self.mp4_set = [
            file[:-4] for file in os.listdir(self.local_dir) if file.endswith("mp4")
        ]
        return super().prepare_dataset(dataset)
...
class LLaVAVideo178KPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        ...
        self.local_dir = MediaResource.download(
            urls, f"llava_video_178k_{self.subset}", file_type="sharded"
        )
        return super().prepare_dataset(dataset)
...
class MovieChat1KPreprocessor(ResponsePreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        mp4_set = (
            ...
            + [f"UWA-{i}.mp4" for i in range(1, 5)]
            + ["UWA-6.mp4"]
        )
        for file in mp4_set:
            if not use_hf_hub():
            ...
            self.local_dir = MediaResource.download(
                url, "moviechat_1k_test", file_type="file"
            )
        return super().prepare_dataset(dataset)
...
def preprocess_mind2web(dataset, **kwargs):
    ...
    def preprocess_row(row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        row["screenshot"] = MediaResource.safe_save(
            screenshot, row["action_uid"] + ".jpg", "mind2web"
        )
        action = row["target_action_reprs"]
        actions = action.split("->")
        row["query"] = (
            f"The snapshot of screen:<image>\nThe html source code:{raw_html}\n"
        )
        action = actions[-1]
    ...
    if isinstance(dataset, HfIterableDataset):
        ...
        return HfIterableDataset.from_generator(
            generate_example, gen_kwargs={"dataset": dataset}
        )
    ...
    if history:
        conversations.append(
            {"messages": history_to_messages(history), "images": images, "tools": tools}
        )
...
class ShareGPT4VPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset):
        split = (
            ...
            if dataset.config_name is None
            else dataset.config_name
        )
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "coco/" in image:
            ...
            image = os.path.join(
                self.all_folders["sam"], image.replace("sam/images/", "")
            )
        elif "llava/" in image:
            image = os.path.join(
                self.all_folders["llava"],
                image.replace("llava/llava_pretrain/images/", ""),
            )
        elif "wikiart/" in image:
            image = os.path.join(
                self.all_folders["wikiart"],
                image.replace("wikiart/images/", "data/wikiart/images/"),
            )
        elif "share_textvqa/" in image:
            image = os.path.join(
                self.all_folders["share_textvqa"],
                image.replace("share_textvqa/images/", "data/share_textvqa/images/"),
            )
        elif "web-celebrity/" in image:
            image = os.path.join(
                self.all_folders["web-celebrity"],
                image.replace("web-celebrity/images/", "data/web-celebrity/images/"),
            )
        elif "web-landmark/" in image:
            image = os.path.join(
                self.all_folders["web-landmark"],
...
class TextCapsEmbPreprocessor(RowPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "positive_messages": [[{"role": "user", "content": row["response"][0]}]],
            "images": row["images"],
        }
...
class TextCapsReRankPreprocessor(RowPreprocessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.negatives_per_sample: int = (
            int(os.environ.get("MAX_NEGATIVE_SAMPLES", 7))
            if negatives_per_sample is None
            else negatives_per_sample
        )
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if self._responses_pool:
            candidates = [
                s
                for s in self._responses_pool
                if isinstance(s, str) and s not in row["response"]
            ]
            if candidates:
...
register_dataset(
    DatasetMeta(
        ...
        subsets=[
            ...
            SubsetDataset(
                name="emb",
                preprocess_func=TextCapsEmbPreprocessor(
                    columns={"reference_strs": "response"}
                ),
                split=["train", "validation"],
            ),
            SubsetDataset(
                name="rerank",
                preprocess_func=TextCapsReRankPreprocessor(
                    columns={"reference_strs": "response"}
                ),
                split=["train", "validation"],
            ),
...
class RefCOCOPreprocessor(ResponsePreprocessor, GroundingMixin):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        image_path = os.path.join(
            self.cache_dir, row["image_path"].replace("coco/train2014", "train2014")
        )
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=MessagesPreprocessor(
            user_role="question", assistant_role="answer"
        ),
        split=["train", "validation"],
        huge_dataset=True,
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=MessagesPreprocessor(
            inner_key="data", user_role="question", assistant_role="answer"
        ),
        tags=["medical", "en", "vqa"],
    )
)
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if os.path.join(self.local_cache, "images", row["imageId"] + ".jpg"):
            return {
                ...
                "images": os.path.join(
                    self.local_cache, "images", row["imageId"] + ".jpg"
                ),
            }
        else:
            return
...
class ClevrPreprocessor(ResponsePreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Dict[str, Any]:
        query = row.get("query", "")
        query = (
            f"{query} Output the thinking process in <think> </think> and "
