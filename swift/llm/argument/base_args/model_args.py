...
class ModelArguments:
    ...
    task_type: Literal[
        "causal_lm", "seq_cls", "embedding", "reranker", "generative_reranker"
    ] = None
    ...
    def _init_device_map(self):
        ...
        if self.device_map:
            self.device_map: Union[str, Dict[str, Any], None] = json_parse_to_dict(
                self.device_map, strict=False
            )
        # compat mp&ddp
    ...
    def _init_max_memory(self):
        ...
        if (
            local_world_size > 1
            and isinstance(self.max_memory, dict)
            and local_rank > 0
        ):
    ...
    def _init_torch_dtype(self) -> None:
        ...
        self.torch_dtype: Optional[torch.dtype] = HfConfigFactory.to_torch_dtype(
            self.torch_dtype
        )
    ...
    def _init_rope_scaling(self):
        ...
        if (
            rope_scaling
            and rope_scaling.get("original_max_position_embeddings") is not None
        ):
            origin_max_model_len = rope_scaling["original_max_position_embeddings"]
        elif self.model_info.rope_scaling:
            if (
                self.model_info.rope_scaling.get("original_max_position_embeddings")
                is not None
            ):
                origin_max_model_len = self.model_info.rope_scaling[
                    "original_max_position_embeddings"
                ]
            elif self.model_info.rope_scaling.get("factor") is not None:
                origin_max_model_len = (
                    self.model_info.max_model_len
                    // self.model_info.rope_scaling["factor"]
                )
        if origin_max_model_len is None:
            origin_max_model_len = self.model_info.max_model_len
        assert (
            origin_max_model_len is not None
        ), "`origin_max_model_len` from model config is not set"
        rope_scaling["original_max_position_embeddings"] = origin_max_model_len

        if "factor" not in rope_scaling:
            rope_scaling["factor"] = max(
                float(math.ceil(self.max_model_len / origin_max_model_len)), 1.0
            )
    ...
    def _init_model_info(self) -> torch.dtype:
        ...
        if (
            self.rope_scaling
            or self.model_info.rope_scaling
    ...
    def __post_init__(self):
        if self.model is None:
            raise ValueError(
                f"Please set --model <model_id_or_path>`, model: {self.model}"
            )
