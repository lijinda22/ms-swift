...
class TemplateArguments:
    ...
    template: Optional[str] = field(
        default=None,
        metadata={"help": f"template choices: {list(TEMPLATE_MAPPING.keys())}"},
    )
    ...
    def __post_init__(self):
        ...
        if self.padding_side is None:
            if getattr(self, "task_type", None) in ("reranker", "generative_reranker"):
                ...
                logger.info(
                    f"Setting args.padding_side to {self.padding_side} for task_type={self.task_type}"
                )
            else:
    ...
    def get_template_kwargs(self):
        ...
        if (
            not isinstance(self, TrainArguments)
            or hasattr(self, "rlhf_type")
            and self.rlhf_type == "grpo"
        ):
