...
class QuantizeArguments:
    ...
    def get_quantization_config(self):
        ...
        if self.quant_method != "fp8" and self.quant_bits is None:
            raise ValueError(
                f"Please set the quant_bits. args.quant_bits: {self.quant_bits}"
            )
        if self.quant_method == "bnb":
            ...
            if self.quant_bits == 8:
                ...
                raise ValueError(
                    "quanto quantization only support quant bits 2/4/8/float8"
                )
    ...
    def __post_init__(self):
        ...
        self.bnb_4bit_compute_dtype: torch.dtype = HfConfigFactory.to_torch_dtype(
