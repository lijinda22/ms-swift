...
class SwiftMixin:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not hasattr(train_dataset, "__len__") and args.dataloader_num_workers > 1:
            ...
            logger.warning(
                "Using IterableDataset, setting args.dataloader_num_workers to 1."
            )
        self.compute_loss_func = (
            None  # Compatible with the older version of transformers
        )
        ...
        if getattr(self.model, "origin_generation_config", None):
            self.model.origin_generation_config.eos_token_id = (
    ...
    def _patch_deepspeed_load_checkpoint(self):
        ...
        if (
            not self.args.resume_from_checkpoint
            or not self.args.resume_only_model
            or not hasattr(trainer, "deepspeed_load_checkpoint")
        ):
            yield
            return
        ...
        def deepspeed_load_checkpoint(*args, **kwargs):
            try:
                ...
                logger.warning(
                    "Failed to call deepspeed_load_checkpoint function. "
    ...
    def get_use_logits_to_keep(self, default_value: bool = True):
        ...
        if use_logits_to_keep is None:
            ...
            use_logits_to_keep = (
                not self.model.model_meta.is_multimodal
    ...
    def _save_initial_model(self, output_dir):
        ...
        if isinstance(model, PeftModel):
            ...
            if isinstance(init_lora_weights, str) and any(
                s in init_lora_weights for s in ("pissa", "olora", "lora-ga")
            ):
        ...
        if is_mp_ddp():
            ...
            for v in self.optimizer.state.values():
                if "step" in v:
                    ...
                    device_set = set([t.device for t in v.values()]) - {
                        v["step"].device,
                        torch.device("cpu"),
                    }
                    if len(device_set) >= 1:
    ...
    def _save_model(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if (
            not isinstance(self.model, supported_classes)
            and self.model.__class__.__name__ not in supported_names
        ):
            ...
            if isinstance(_unwrap_model, supported_classes):
                ...
                if use_flash_ckpt:
                    self.flash_checkpointer.ckpt_agent.save(
        ...
        elif AutoModelForCausalLMWithValueHead and isinstance(
            ...
            if save_safetensors:
                ...
                torch.save(
                    v_head_state_dict, os.path.join(output_dir, "value_head.bin")
                )
            ...
            if self.model.__class__.__name__ != "SentenceTransformer":
                ...
                def save_context():
                    ...
                    _state_dict = {
                        (
                            key[len("0.auto_model.") :] if "auto_model" in key else key
                        ): value
                ...
                with save_context():
                    if use_flash_ckpt:
                        ...
                        self.model.save_pretrained(
                            output_dir, safe_serialization=save_safetensors
                        )
    ...
    def _fix_zero3_gather_all_parameters(self) -> None:
        if is_deepspeed_zero3_enabled() and not hasattr(
            ...
            if "exclude_frozen_parameters" in parameters:
                ...
                def _zero3_consolidated_16bit_state_dict(
                    model, exclude_frozen_parameters=False
                ):
                    unwrapped = unwrap_model(model)
                    exclude_frozen_parameters = False
                    if (
                        isinstance(unwrapped, SwiftModel)
                        and unwrapped.has_additional_modules
                    ):
                        exclude_frozen_parameters = True
                    if isinstance(unwrapped, PeftModel):
                        exclude_frozen_parameters = True
                    return model._zero3_consolidated_16bit_state_dict_origin(
    ...
    def _patch_tasks(self):
        ...
        if "SentenceTransformer" in model.__class__.__name__:
            ...
            def forward_transformer(
                transformer, features: Dict[str, torch.Tensor], **kwargs
                ...
                outputs = transformer.auto_model(
                    **trans_features, **kwargs, return_dict=True
                )
                token_embeddings = outputs[0]
                features["token_embeddings"] = token_embeddings

                if (
                    transformer.auto_model.config.output_hidden_states
                    and "hidden_states" in outputs
                ):
            ...
            def forward_sentence_transformer(
                sentence_transformer, **kwargs
            ) -> Dict[str, torch.Tensor]:
                input = kwargs
                kwargs = {}
                for idx, (module_name, module) in enumerate(
                    sentence_transformer.named_children()
                ):
                    ...
                    if isinstance(module, Router):
                        ...
                        if sentence_transformer.module_kwargs is not None:
                            module_kwarg_keys = sentence_transformer.module_kwargs.get(
                                module_name, []
                            )
                        module_kwargs = {
                            key: value
                    ...
                    if idx == 0 and self.args.padding_free:
                        output = revert_padding_free(
                            output, input, self.args.padding_side
                        )
                    input = output
            ...
            if self.args.task_type == "embedding":
                llm_model = get_lm_head_model(
                    self.model, model_meta=self.model.model_meta
                )

                def revert_padding_free_hook(module, args, input, output):
                    return revert_padding_free(output, input, self.args.padding_side)

                llm_model.register_forward_hook(
                    revert_padding_free_hook, with_kwargs=True, prepend=True
                )
                ...
                def seq_cls_forward(model, *args, **kwargs):
                    ...
                    def inner_forward(*args, **kwargs):
                        ...
                        return revert_padding_free(
                            output, kwargs, self.args.padding_side
                        )
    ...
    def train(self, *args, **kwargs):
        if self.model_meta.is_multimodal:
            ...
            models = list(
                set(self.accelerator.unwrap_model(model) for model in models)
            )  # Deduplicate
            self.template.register_post_encode_hook(models)
            logger.info(
                f"Successfully registered post_encode hook: {[model.__class__.__name__ for model in models]}."
            )
        ...
        with self.hub.patch_hub(), self._fix_grad_norm_nan(), self._patch_skip_first_batches(), self._patch_deepspeed_load_checkpoint():
    ...
    def create_loss_and_metric(self, args):
        ...
        if args.metric is not None:
            res["compute_metrics"], res["preprocess_logits_for_metrics"] = get_metric(
                args.metric
            )
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        if self.args.optimizer is not None:
            ...
            self.optimizer, self.lr_scheduler = optimizer_callback(
                self.args, self.model, self.train_dataset
            )
            ...
            super().create_optimizer_and_scheduler(
    ...
    def _compute_acc(self, outputs, labels) -> None:
        ...
        if (
            ...
            in {"generative_reranker", "listwise_generative_reranker"}
            ...
            if tokenizer is None:
                raise RuntimeError(
                    "tokenizer not available for generative_reranker acc"
                )
            ...
            if (
                isinstance(positive_token_id, int)
                and isinstance(negative_token_id, int)
                and positive_token_id >= 0
                and negative_token_id >= 0
            ):
            ...
            if self.template.sequence_parallel_size > 1:
                ...
                if sequence_parallel.rp_world_size > 1:
                    ...
                    position_ids = None
                preds_output = sequence_parallel.gather(
                    preds, dim=1, position_ids=position_ids
                )
                labels_output = sequence_parallel.gather(
                    labels, dim=1, position_ids=position_ids
                )
    ...
    def prepare_logits_to_keep(self, inputs):
        ...
        if labels.shape[0] == 1 and not is_mp():
            ...
            logits_to_keep = (
                labels.shape[-1] - ((labels != -100).int().argmax(-1).min().item()) + 1
            )
    ...
    def get_sp_dataloader(self, dataset, batch_size, skip_batches=0):
        ...
        if hasattr(dataset, "__len__"):
            ...
            return DataLoaderShard(
                dataset, device=self.accelerator.device, **dataloader_params
            )
        else:
            dataloader_params = {
                "collate_fn": data_collator,
            ...
            if dist.is_initialized() and dataloader_params["prefetch_factor"]:
                dataloader_params["prefetch_factor"] = (
                    dataloader_params["prefetch_factor"] * dist.get_world_size()
                )
    ...
    def get_train_dataloader(self, skip_batches=0):
        ...
        if self.template.sequence_parallel_size > 1:
            dataloader = self.get_sp_dataloader(
                self.train_dataset, self._train_batch_size, skip_batches=skip_batches
            )
            ...
            if hasattr(train_dataset, "__len__"):
                ...
                if skip_batches > 0:
                    ...
                    batch_sampler = SkipBatchSampler(
                        batch_sampler, skip_batches=skip_batches
                    )
