...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_model_tokenizer(self, **kwargs):
        ...
        if getattr(args, "kd_teacher_model_type", None):
            ...
            for t_type, t_path in zip(kd_types, kd_paths):
                ...
                t_transform = None
                if t_type == "conchv1_5":
                    t_model, t_transform = create_conchv1_5()
                elif t_type == "uni2":
                    t_model = get_encoder_uni2()
                    t_transform = get_eval_transforms_uni()
                elif t_type == "virchow2":
                    t_model = get_encoder_virchow2()
                    t_transform = get_eval_transforms_virchow2()
                else:
                    raise ValueError(f"Unknown KD teacher type: {t_type}")
                assert t_model, "Failed to load KD teacher model"
            ...
            self.model.teacher_transforms = self.teacher_transforms
    ...
    def _prepare_template(self) -> None:
        ...
        if (
            ...
            and (args.padding_free or args.packing)
        ...
        if self.teacher_models and len(self.teacher_models) > 0:
    ...
    def _prepare_dataset(self):
        ...
        pre_process = not (
            hasattr(args, "rlhf_type") and args.rlhf_type in ["grpo", "gkd"]
        )
    ...
    def _post_process_datasets(self, datasets: List) -> List:
        ...
        for i, dataset in enumerate(datasets):
            ...
            if i == 1 and predict_with_generate:
                ...
                continue
            if (
                args.model_meta.is_multimodal or args.lazy_tokenize
    ...
    def _prepare_callbacks(self):
        ...
        if args.early_stop_interval is not None and args.early_stop_interval > 0:
            ...
            logger.info(
                "You are using the default early stop callback, this is a implementation of "
        ...
        for i, dataset in enumerate(datasets):
            ...
            if not args.lazy_tokenize and not args.streaming:
                preprocessor = EncodePreprocessor(
                    template=template, pre_tokenize=args.model_meta.is_multimodal
                )
