...
class WSIEmbeddingDataset(Dataset):
    def __init__(
        ...
        self.directory_depth = infer_folder_structure(
            self.data_source, self.subdir_name
        )
    ...
    def get_feat_path(self, slide_id):
        if self.directory_depth == 2:
            feat_path = os.path.join(
                self.data_source, self.subdir_name, slide_id + self.ext
            )
        else:
            feat_path = os.path.join(self.data_source, slide_id + self.ext)
        return feat_path
